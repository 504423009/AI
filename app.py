...
def generate():
    ...
    # 提示词后缀
        # ==================== 优化后的 Prompt + 调用 ====================
    # 强约束：让模型尽量保留原产品
    preserve = "保持产品完全不变, exact same product, identical appearance, shape, color, texture, branding, logo, packaging, exact proportions, do not change or redraw the product itself, only edit background and lighting"

    if platform == 'amazon':
        suffix = "pure white background, studio lighting, soft even shadows, professional product photography, commercial advertising style, clean minimalist, high resolution"
    else:
        suffix = "clean light grey gradient background, professional product photography, soft studio lighting"

    final_main_prompt = f"{preserve}, {main_prompt}, {suffix}, 8k details, sharp focus"
    final_variant_prompt = f"{preserve}, {variant_prompt}, high quality, photorealistic, product photography, clean background"
    ...
    for i in range(main_count):
        img_url = generate_image(
            ...
            strength=0.32          # 主图建议 0.25-0.35，越低保留越强
        )
        if img_url:
            try:
